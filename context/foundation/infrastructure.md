...
- **CI auto-deploys on push to `main`.** `.github/workflows/ci.yml` keeps lint, test, and build on PRs and `main`, then a `deploy` job (`needs: ci`) runs `wrangler deploy` only for `push` to `main`.

## Operational Story

- **Preview deploys**: Local production-like check is `npm run build && npm run preview` (`workerd`). Git-connected Workers Builds use `npx wrangler versions upload` on non-main branches and can post `*.workers.dev` preview URLs; that requires `preview_urls: true` in Wrangler (this repo sets `preview_urls: false`) and is **workers.dev only** (no custom domain on previews). GitHub Actions do not publish previews. Treat preview URLs as public unless Cloudflare Access is added. Fork PRs should not receive production secrets.
