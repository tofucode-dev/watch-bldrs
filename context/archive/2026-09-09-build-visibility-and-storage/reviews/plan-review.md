...
- **Decision**: FIXED — Applied user approach: SQL allows `published → draft`; MVP UI does not expose unpublish. Updated plan Phase 2 plus PRD, roadmap, testing.md, AGENTS.md, modules.md, security.md, change.md, and plan-brief. Added Progress 2.5.
...
- **Decision**: FIXED — Fixed via Fix A. Wrote security.md “CI verification exception” (§16 six fields, CI-only). Pointed OPERATIONAL_SAFETY §15 at it. AGENTS.md DoD requires local `db:types` and `test:integration`. Plan Phase 2 + Progress 2.6. SQL unpublish stays the product contract from F1.
...
- **Decision**: FIXED — Added list(), non-owner UPDATE/DELETE deny, and upload-before-row to Phase 3 Storage contract (covered by 3.1).
...
- **Decision**: FIXED — Pinned those max lengths in Phase 1 and Phase 2 data-model sync.
...
- **Decision**: FIXED — `build_parts.id` now `uuid pk default gen_random_uuid()`.
...
- **Decision**: FIXED — Phase 1 now REVOKE ALL from public, then table grants, plus GRANT USAGE ON TYPE for every new enum to anon and authenticated.
...
- **Decision**: FIXED — Narrowed 2.4; added Phase 3 manual 3.5 for README `test:integration` follow-through.
